...
class DTAgent():
    def __init__(self, dataset=None, scorer="balanced_accuracy"):
        ...
        self.optimal_params = {}
        self.param0name = "max_depth"
        self.param1name = "ccp_alpha"
        ...
        self.param_distribution = {
            self.param0name: [1, 2, 5, 10, 15, 20, 25, 30, 40, 50, 75, 100, 150, 200, 500],
            self.param1name: [0.0, 0.005, 0.010, 0.015, 0.02, 0.025, 0.030, 0.035, 0.04, 0.05, 0.06, 0.07, 0.08, 0.09, 0.1]
        }
    ...
    def initModel(self, x_train, y_train):
        ...
        #            ('clf', DecisionTreeClassifier(max_depth= self.optimal_params[self.param0name], ccp_alpha= self.optimal_params[self.param1name] , random_state=0, criterion='entropy'))
        #            ])
        self.model = Pipeline([
                    ('normalizer', StandardScaler()),
                    ('clf', DecisionTreeClassifier(max_depth= 40, ccp_alpha= 0.01 , random_state=0, criterion='gini'))
                    ])
    ...
    def save_final_params(self):
        ...
        final_df.to_csv('./results/final_{}_{}_metrics.csv'.format('DT',self.dataset))
    ...
    def find_optimal_params(self, x_train, y_train):
        ...
        #    DecisionTreeClassifier(), param_distributions=param_distribution, 
        #    scoring= "accuracy", n_iter=200, random_state=0, n_jobs=2,
        #)

        search_cv = GridSearchCV(
            DecisionTreeClassifier(random_state=0, criterion='entropy'), self.param_distribution, scoring=self.scoreMethod, cv=5, refit=True)
        ...
        cv_results.to_csv('./results/{}_{}_tuning.csv'.format('DT',self.dataset))
        ...
        # self.optimal_params["max_depth"] = sorted.iloc[0][0]
        # self.optimal_params["ccp_alpha"] = sorted.iloc[0][1]

        #self.optimal_params["max_depth"] = sorted.iloc[0][0]
        #self.optimal_params["ccp_alpha"] = sorted.iloc[0][1]

        self.optimal_params["max_depth"] = search_cv.best_params_[self.param0name]
        self.optimal_params["ccp_alpha"] = search_cv.best_params_[self.param1name]
    ...
    def plot_learning_timing_curve(self, x_train, y_train):
        ...
        scores_df.to_csv('./results/{}_{}_LC_stats.csv'.format('DT',self.dataset))
        ...
        plt.legend()
        plt.savefig('./graphs/dtGraphs/{}_{}_{}.png'.format(self.modelType, 'LearningCurve',self.dataset))
        ...
        plt.ylabel("Time (ms)")
        plt.legend()
        plt.savefig('./graphs/dtGraphs/{}_{}_{}.png'.format(self.modelType, 'TimingCurve',self.dataset))
    ...
    def plot_validation_curve(self, x_train, y_train):
        model = sklearn.base.clone(self.model)
        train_scores, val_scores = validation_curve(model, x_train, y_train, scoring=self.scoreMethod, cv=5, param_name="clf__max_depth", param_range=self.param_distribution[self.param0name])
        final_train_scores = np.mean(train_scores, axis=1)
        final_train_scores_std = np.std(train_scores, axis=1)
        final_val_scores = np.mean(val_scores, axis=1)
        final_val_scores_std = np.std(val_scores, axis=1)
        ...
        plt.ylabel("Score ({})".format(self.scoreMethod))
        plt.legend()
        plt.savefig('./graphs/dtGraphs/{}_{}_{}.png'.format('VC', self.param0name, self.dataset))


        model = sklearn.base.clone(self.model)
        train_scores, val_scores = validation_curve(model, x_train, y_train, scoring=self.scoreMethod, cv=5, param_name="clf__ccp_alpha", param_range=self.param_distribution[self.param1name])
        final_train_scores = np.mean(train_scores, axis=1)
        final_train_scores_std = np.std(train_scores, axis=1)
        final_val_scores = np.mean(val_scores, axis=1)
        final_val_scores_std = np.std(val_scores, axis=1)
        ...
        plt.ylabel("Score ({})".format(self.scoreMethod))
        plt.legend()
        plt.savefig('./graphs/dtGraphs/{}_{}_{}.png'.format('VC', self.param1name, self.dataset))
