...
class NNAgent():
    def __init__(self, dataset=None, scorer="balanced_accuracy"):
        ...
        self.param_distribution = {
            self.param0name: [10, 25, 50, 100],
            self.param1name: [0.0, 0.0001, 0.0003, 0.0005]
        }
    ...
    def initModel(self, x_train, y_train):
        ...
        #self.model = Pipeline([
        #            ('normalizer', StandardScaler()),
        #            ('clf', MLPClassifier(hidden_layer_sizes= 40, alpha= 0.0001 , hidden_layer_sizes=100, random_state=0)
        #            ])
