...
class SVMAgent():
    def __init__(self, dataset=None, scorer="balanced_accuracy"):
        ...
        self.param_distribution = {
            self.param0name: [.1,.3,.5,.75,1],
            self.param1name: [1,2,3,4,5]
        }
    ...
    def initModel(self, x_train, y_train):
        ...
        self.model = Pipeline([
                    ...
                    ('clf', SVC(C= self.optimal_params[self.param0name], degree= self.optimal_params[self.param1name]))
                    ])
        #self.model = Pipeline([
        #            ('normalizer', StandardScaler()),
        #            ('clf', SVC(C= 1, degree= 3))
        #            ])
    ...
    def save_final_params(self):
        ...
        final_df.to_csv('./results/final_{}_{}_metrics.csv'.format('SVM',self.dataset))
    ...
    def find_optimal_params(self, x_train, y_train):
        ...
        #    SVC(), param_distributions=param_distribution, 
        #    scoring= "accuracy", n_jobs=2,
        #)

        search_cv = GridSearchCV(
            SVC(), self.param_distribution, scoring=self.scoreMethod, cv=5, refit=True)
        ...
        cv_results.to_csv('./results/{}_{}_tuning.csv'.format('SVM',self.dataset))
        ...
        # self.optimal_params["C"] = sorted.iloc[0][0]
        # self.optimal_params["degree"] = sorted.iloc[0][1]

        #self.optimal_params["C"] = sorted.iloc[0][0]
        #self.optimal_params["degree"] = sorted.iloc[0][1]

        self.optimal_params["C"] = search_cv.best_params_[self.param0name]
        self.optimal_params["degree"] = search_cv.best_params_[self.param1name]
    ...
    def plot_learning_timing_curve(self, x_train, y_train):
        ...
        scores_df.to_csv('./results/{}_{}_LC_stats.csv'.format('SVM',self.dataset))
        ...
        plt.legend()
        plt.savefig('./graphs/svmgraphs/{}_{}_{}.png'.format(self.modelType, 'LearningCurve',self.dataset))
        ...
        plt.ylabel("Time (ms)")
        plt.legend()
        plt.savefig('./graphs/svmgraphs/{}_{}_{}.png'.format(self.modelType, 'TimingCurve',self.dataset))
    ...
    def plot_validation_curve(self, x_train, y_train):
        model = sklearn.base.clone(self.model)
        train_scores, val_scores = validation_curve(model, x_train, y_train, scoring=self.scoreMethod, cv=5, param_name="clf__C", param_range=self.param_distribution[self.param0name])
        final_train_scores = np.mean(train_scores, axis=1)
        final_train_scores_std = np.std(train_scores, axis=1)
        final_val_scores = np.mean(val_scores, axis=1)
        final_val_scores_std = np.std(val_scores, axis=1)
        ...
        plt.ylabel("Score ({})".format(self.scoreMethod))
        plt.legend()
        plt.savefig('./graphs/svmgraphs/{}_{}_{}.png'.format('VC', self.param0name, self.dataset))


        model = sklearn.base.clone(self.model)
        train_scores1, val_scores1 = validation_curve(model, x_train, y_train, scoring=self.scoreMethod, cv=5, param_name="clf__degree", param_range=self.param_distribution[self.param1name])
        final_train_scores = np.mean(train_scores, axis=1)
        final_train_scores_std = np.std(train_scores, axis=1)
        final_val_scores = np.mean(val_scores, axis=1)
        final_val_scores_std = np.std(val_scores, axis=1)
        ...
        plt.ylabel("Score ({})".format(self.scoreMethod))
        plt.legend()
        plt.savefig('./graphs/svmgraphs/{}_{}_{}.png'.format('VC', self.param1name, self.dataset))
